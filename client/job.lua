                ...
                for k, v in pairs(result) do
                    ...
                    elseif result["BLEED"] > 0 then
                      ...
                      --  })
                        ...
                        QBCore.Functions.Notify('Kişi Sağlıklı', 'success')
                    end
                end
...
RegisterNetEvent('hospital:client:RevivePlayer', function()
            ...
            if player ~= -1 and distance < 5.0 then
                ...
                QBCore.Functions.Progressbar("hospital_revive", "Kişiye Tıbbi Müdahale Yapılıyor..", 5000, false, true, {
                    ...
                    QBCore.Functions.Notify("Müdahale bitti")
                    TriggerServerEvent("hospital:server:RevivePlayer", playerId)
                end, function() -- Cancel
                    isHealingPerson = false
                    StopAnimTask(PlayerPedId(), healAnimDict, "exit", 1.0)
                    QBCore.Functions.Notify("Vazgeçildi!", "error")
                end)
            else
                QBCore.Functions.Notify("Yakınında kimse yok", "error")
            end
    ...
    QBCore.Functions.TriggerCallback('QBCore:HasItem', function(hasItem)
        if hasItem then
            ...
            if player ~= -1 and distance < 5.0 then
                local playerId = GetPlayerServerId(player)
                isHealingPerson = true
                QBCore.Functions.Progressbar("hospital_healwounds", "Yaraları sarıyorsun..", 5000, false, true, {
                    disableMovement = false,
                    disableCarMovement = false,
                    disableMouse = false,
                    disableCombat = true,
...
CreateThread(function()
    ...
    while true do
        ...
        if inZone and not alreadyEnteredZone then
            ...
            TriggerEvent('text:show', "<b style='color:#44e467;'>[E] </b>Depo", "")
        end
        ...
        if LocalPlayer.state.isLoggedIn then
                    ...
                    if dist < 4.5 then
                        if onDuty then
                            if dist < 1.5 then
                                ...
                                inZone  = true
                                text = 'Eşya Marketi'
                                --DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Cephane")
                                if IsControlJustReleased(0, 38) then
                                    TriggerEvent('text:hide')
                    ...
                    if dist < 4.5 then
                        ...
                        if dist < 1.5 then
                            ...
                            if IsPedInAnyVehicle(ped, false) then
                                text = 'Aracı Koy'
                                --rawText3D(v.x, v.y, v.z, "~g~E~w~ - Garaj")
                            else
                                text = 'Garaj'
                                --DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Aracı Koy")
                            end
                            if IsControlJustReleased(0, 38) then
                                TriggerEvent('text:hide')
                                if IsPedInAnyVehicle(ped, false) then
                    ...
                    if dist < 7.5 then
                        if onDuty then
                            ...
                            if dist < 2.5 then
                                ...
                                if IsPedInAnyVehicle(ped, false) then
                                    text = 'Aracı Koy'
                                    --DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Garaj")
                                else
                                    text = 'Garaj'
                                    --DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Aracı Koy")
                                end
            ...
            for k, v in pairs(Config.Locations["main"]) do
                ...
                if dist < 1.5 then
                    ...
                    inZone  = true
                    text = 'Asansör'
                    --DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Asansör")
                    if IsControlJustReleased(0, 38) then
                        Menu()
                    end
                end
...
RegisterNetEvent("ambulance:roof", function(data)
    local ped = PlayerPedId()
    DoScreenFadeOut(500)
    ...
    if data.altkat then
        loc = "altkat" 
    elseif data.cati then 
        loc = "roof"
    elseif data.ortakat then 
        loc = "main"
    end
    local coords = Config.Locations[loc][1]
