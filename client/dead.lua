...
CreateThread(function()
	while true do
		...
		if NetworkIsPlayerActive(player) then
            ...
            if IsEntityDead(playerPed) and not InLaststand and isDead then
                 SetLaststand(false)
                ...
                killerIDdc = GetPlayerServerId(killerId)
                TriggerServerEvent('um-deathlog:server:ambulance',playername,playerid,killername,weaponlabel,weaponname,killerIDdc)               
                deathTime = Config.DeathTime
                OnDeath()
                DeathTimer()
            elseif IsEntityDead(playerPed) and not InLaststand and not isDead then
                SetLaststand(false)
                local killer_2, killerWeapon = NetworkGetEntityKillerOfPlayer(player)
                local killer = GetPedSourceOfDeath(playerPed)
		...
		if isDead or InLaststand then
            ...
            if isDead then
                if not isInHospitalBed then
                    if deathTime > 0 then
                        DrawTxt(0.93, 1.44, 1.0,1.0,0.4, "yeniden dogmak için: ~r~" .. math.ceil(deathTime) .. "~w~ saniye", 255, 255, 255, 255)
                    else
                        DrawTxt(0.93, 1.44, 1.0,1.0,0.4, "~g~[E] ("..hold..")~w~ yeniden dog ~r~($"..Config.BillCost..")~w~", 255, 255, 255, 255)
                    end
                    if not emsNotified then
                        DrawTxt(0.95, 1.40, 1.0,1.0,0.4, " ~g~[G]~w~ bildirim gönder", 255, 255, 255, 255)
                    end
                    if IsControlJustPressed(0, 47) and not emsNotified then
                        local coords = GetEntityCoords(ped)
                        local data = {displayCode = 'İhbar EMS', description = "Yaralı İhbarı", isImportant = 0, recipientList = {'ambulance'}, length = '10000', infoM = 'fa-info-circle', info = "Yaralı var!"}
            ...
            elseif InLaststand then
                ...
                if LaststandTime > Laststand.MinimumRevive then
                    DrawTxt(0.93, 1.44, 1.0,1.0,0.4, "yeniden dogmak için: ~r~" .. math.ceil(LaststandTime) .. "~w~ saniye", 255, 255, 255, 255)
                    if not emsNotified then
                        DrawTxt(0.95, 1.40, 1.0,1.0,0.4, " ~g~[G]~w~ bildirim gönder", 255, 255, 255, 255)
                    end
