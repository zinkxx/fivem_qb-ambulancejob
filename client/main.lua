...
CreateThread(function()
    while true do
        ...
        if isInHospitalBed and canLeaveBed then
            sleep = 0
            local pos = GetEntityCoords(PlayerPedId())
            DrawText3D(pos.x, pos.y, pos.z, "~g~E~w~ - Yatakdan kalk..")
...
RegisterNetEvent("ambulance:client:checkin", function()
    TriggerEvent('animations:client:EmoteCommandStart', {"notal"})
    QBCore.Functions.Progressbar("hospital_checkin", "Kaydınız yapılıyor..", 2000, false, true, {
        ...
        if bedId then
            ...
            QBCore.Functions.Notify("Yataklar dolu..", "error")
        end
    end, function() -- Cancel
        TriggerEvent('animations:client:EmoteCommandStart', {"c"})
        QBCore.Functions.Notify("Kayıt başarısız oldu!", "error")
    end)
end)

local UseBeds = false
RegisterNetEvent("um:ambulance:Bed", function()
    for k,v in pairs(Config.UMBeds) do
        local PlayerPed = PlayerPedId()
        local PlayerPos = GetEntityCoords(PlayerPed)
        local dists = #(PlayerPos - vec3(v.x,v.y,v.z))
        if dists < 2 then
            UseBeds = true
            FreezeEntityPosition(PlayerPed, true)
            SetEntityCoords(PlayerPed, v.x, v.y, v.z - 1.0)
            SetEntityHeading(PlayerPed, v.w)
            Wait(100)
            TriggerEvent('animations:client:EmoteCommandStart', {"uzanmak"})
        end
    end
end)

RegisterNetEvent("um:ambulance:Bed2", function()
    for k,v in pairs(Config.UMBeds) do
        local PlayerPed = PlayerPedId()
        local PlayerPos = GetEntityCoords(PlayerPed)
        local dists = #(PlayerPos - vec3(v.x,v.y,v.z))
        if dists < 2 then
            ...
            SetEntityHeading(PlayerPed, v.w)
            Wait(100)
            TriggerEvent('animations:client:EmoteCommandStart', {"güneşlen3"})
        end
    end
end)
