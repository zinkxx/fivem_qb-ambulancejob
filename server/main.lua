	...
	for k, v in pairs(Config.Locations["beds"]) do
		if not v.taken then
			...
			if Config.WipeInventoryOnRespawn then
				...
				TriggerClientEvent('QBCore:Notify', src, 'Tüm eşyalarınız alındı..', 'error')
			end
			Player.Functions.RemoveMoney("bank", Config.BillCost, "respawned-at-hospital")
			TriggerEvent('qb-bossmenu:server:addAccountMoney', "ambulance", Config.BillCost)
			--TriggerClientEvent('hospital:client:SendBillEmail', src, Config.BillCost)
...
RegisterNetEvent('hospital:server:', function(data)
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
	if Player then
	...
	if Patient then
		if oldMan then
			if Player.Functions.RemoveMoney("cash", 5000, "revived-player") then
				...
				TriggerClientEvent('QBCore:Notify', src, "Yeterli paraya sahip değilsin", "error")
			end
		else
			--Player.Functions.RemoveItem('firstaid', 1)
			--TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['firstaid'], "remove")
...
RegisterNetEvent('hospital:server:SendDoctorAlert', function()
    ...
    for k,v in pairs(players) do
        if v.PlayerData.job.name == 'ambulance' and v.PlayerData.job.onduty then
			TriggerClientEvent('QBCore:Notify', v.PlayerData.source, 'Hastanede bir doktora ihtiyaç var', 'ambulance')
		end
	end
end)
...
RegisterNetEvent('hospital:server:CanHelp', function(helperId, canHelp)
	...
	if canHelp then
		...
		TriggerClientEvent('QBCore:Notify', helperId, "Bu kişiye yardım edemezsiniz..", "error")
	end
end)
...
QBCore.Commands.Add('911e', 'EMS İhbar', {{name='mesaj', help='Bildirim mesajı'}}, false, function(source, args)
	local src = source
	...
	local name = character.PlayerData.charinfo.firstname ..' '.. character.PlayerData.charinfo.lastname
			local data = {displayCode = 'İhbar EMS', description = "İhbar Eden: " ..name, isImportant = 0, recipientList = {'ambulance','police'}, length = '10000', infoM = 'fa-info-circle', info = message}
...
QBCore.Commands.Add('heal', 'Heal A Player or Yourself (Admin Only)', {{name='id', help='Player ID (may be empty)'}}, false, function(source, args)
	local src = source
	if args[1] then
		local Player = QBCore.Functions.GetPlayer(tonumber(args[1]))
		if Player then
...
RegisterNetEvent('um-deathlog:server:ambulance', function(playername,playerid,killername,weaponlabel,weaponname,killerIDdc)
	-- QBCore.Functions.GetIdentifier(src, 'discord')
	if killerIDdc ~= 0 then
	TriggerEvent("qb-log:server:CreateLog", "death",Lang:t('logs.death_log_title', {playername,playerid}), "red"," **Öldüren: **" ..killername.. "\n**Discord:** " ..(QBCore.Functions.GetIdentifier(killerIDdc, 'discord') or 'undefined').. "\n **Steam: **"..(QBCore.Functions.GetIdentifier(killerIDdc, 'steam') or 'undefined').. "\n **License: **"..(QBCore.Functions.GetIdentifier(killerIDdc, 'license') or 'undefined').. "\n\n\n **Öldürülen: **" ..playername.. "\n **Discord: **"..(QBCore.Functions.GetIdentifier(playerid, 'discord') or 'undefined').. "\n **Steam: **"..(QBCore.Functions.GetIdentifier(playerid, 'steam') or 'undefined').. "\n **License: **"..(QBCore.Functions.GetIdentifier(playerid, 'license') or 'undefined').."\n\n **Silah: **"..weaponname )
	end
end)